...
function latex(src, options) {
  ...
  temp.mkdir('node-latex', (err, tempPath) => {
    ...
    /**
     * Run a LaTeX child process on the document stream
...
// Function convert from file.tex to file.pdf

function convert(code) {
  const input = createReadStream(code)
  const output = createWriteStream(join(__dirname, 'output/output.pdf'))
...
app.post('/convert', (req, res) => {
  const inputLink = join(__dirname, 'data/hamso2.tex')
  // decode data 
  // const data = Buffer.from(req.body.source_code, 'base64').toString('ascii')
  // create file.tex
  fs.writeFile(inputLink, req.body.source_code, function (err) {
    if (err) return console.log(err)
    console.log('File created')
  })
  // convert from file.tex to file.pdf
  convert(inputLink)
